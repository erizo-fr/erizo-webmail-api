...
module.exports.getMessages = function * () {
	...
	this.checkQuery('ids').optional().notEmpty().match(imapHelper.PATTERN_IDS);
	this.checkQuery('seqs').optional().notEmpty().match(imapHelper.PATTERN_IDS);
	this.checkQuery('bodies').optional().match(imapHelper.PATTERN_BODIES);
...
module.exports.getMessage = function * () {
	this.checkParams('box').notEmpty().match(imapHelper.PATTERN_BOX);
	this.checkParams('id').notEmpty().match(imapHelper.PATTERN_ID);
	this.checkQuery('bodies').optional().match(imapHelper.PATTERN_BODIES);
	this.checkQuery('markSeen').optional().default('false').toBoolean();
	this.checkQuery('fetchStruct').optional().default('false').toBoolean();
	this.checkQuery('fetchEnvelope').optional().default('false').toBoolean();
	...
	this.body = JSON.stringify(message, null, '\t');
	this.status = 200;
};
