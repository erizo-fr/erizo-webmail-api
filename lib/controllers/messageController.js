"use strict"
let MODULE_NAME = "MessageController"
...
let imapHelper = require("../helpers/imapHelper")
...
module.exports.postMessage = function * () {
	...
	let sendMessageTask = messageService.sendMessage(smtpConnection, message)

	logger.debug("Getting the sent box name")
	let sentBox = yield imapHelper.getSentBox(imapConnection)

	logger.info("Posting new message")
	let result = null
	if (!sentBox) {
		logger.warn("The sent box is undefined, the message can not be appended into the sent box")
		result = yield sendMessageTask
	} else {
		logger.debug("The sent box is defined, the message can be appended into the sent box")
		let appendSentMessageTask = messageService.appendMessage(imapConnection, sentBox, message)
		result = yield [sendMessageTask, appendSentMessageTask]
		result = result[0]
	}
...
module.exports.getMessagePartContent = function * () {
	...
	if (attributes) {
		...
		if (attributes.params && attributes.params.charset) {
			charset = attributes.params.charset
		}
	...
	this.status = 200
	this.type = (type && subtype ? type + "/" + subtype : contentType) + (charset ? "; charset=" + charset : "")
	logger.debug("Serving part content : " + filename + " (" + this.type + ")")
	if (encoding) {
