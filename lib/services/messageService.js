 ...
 * Send message
 */
module.exports.sendMessage = function * (smtpConnection, message) {
	// Send message
 ...
 * Append message
 */
module.exports.appendMessage = function * (imapConnection, box, message) {
	let boxPath = box.path
	logger.info("Appending message to box#" + boxPath)
	...
	logger.debug("Appending the sent message in the box#" + boxPath)
...
module.exports.getMessages = function * (imapConnection, boxName, ids, seqs, options) {
	...
	// Open the box
	yield Q.ninvoke(imapConnection, "openBox", boxName, false)
